/*  =========================================================================
    fty_sensor_gpio_selftest.c - run selftests
...
// Tests for draft public classes:
    { "libgpio", libgpio_test, false, true, NULL },
...
{
    test_item_t *item;
    printf ("Running fty-sensor-gpio selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_SENSOR_GPIO_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_sensor_gpio_private_selftest (verbose, item->subtest);
#endif // FTY_SENSOR_GPIO_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_sensor_gpio_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-sensor-gpio test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_SENSOR_GPIO_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_sensor_gpio_private_selftest (verbose, test->subtest);
#endif // FTY_SENSOR_GPIO_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
